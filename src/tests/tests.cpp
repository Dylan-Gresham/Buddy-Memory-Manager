...
#include "../buddy_memory_manager.hpp"

void check_buddy_pool_full(BuddyPool* pool) {
    for (int i = 0; i < pool->kval_m; i++) {
        Avail* avail = &pool->avail[i];
        assert(reinterpret_cast<void*>(avail->next) == reinterpret_cast<void*>(avail));
        assert(reinterpret_cast<void*>(avail->prev) == reinterpret_cast<void*>(avail));
        assert(avail->tag == BLOCK_UNUSED);
        assert(static_cast<int>(avail->kval) == i);
    }
    ...
    assert(top->next == reinterpret_cast<Avail*>(pool->base));
}
...
void test_buddy_malloc_one_large() {
    ...
    Avail* block = reinterpret_cast<Avail*>(reinterpret_cast<char*>(mem) - sizeof(Avail));
    assert(static_cast<int>(block->kval) == MIN_K);
...
void test_buddy_calc_basic_pairs() {
    ...
    Avail* block2 = reinterpret_cast<Avail*>(reinterpret_cast<char*>(block1) + (1 << kval));
    block1->kval = kval;
    block2->kval = kval;
    ...
    size_t offset1 = reinterpret_cast<char*>(block1) - reinterpret_cast<char*>(pool.base);
    size_t offset2 = reinterpret_cast<char*>(block2) - reinterpret_cast<char*>(pool.base);
...
void test_buddy_calc_recursive_coalescing() {
    ...
    Avail* right1 = reinterpret_cast<Avail*>(reinterpret_cast<char*>(left1) + (1 << k1));
    left1->kval = k1;
    right1->kval = k1;

    int k2 = k1 - 1;
    Avail* left2 = left1;
    Avail* right2 = reinterpret_cast<Avail*>(reinterpret_cast<char*>(left2) + (1 << k2));
    left2->kval = k2;
    right2->kval = k2;

    int k3 = k2 - 1;
    Avail* left3 = left2;
    Avail* right3 = reinterpret_cast<Avail*>(reinterpret_cast<char*>(left3) + (1 << k3));
    left3->kval = k3;
    right3->kval = k3;
    ...
    size_t offset_diff = reinterpret_cast<char*>(buddy) - reinterpret_cast<char*>(merged_block);
